...
function loadSectionData(sectionName) {
    switch(sectionName) {
        case 'dashboard':
            // Dashboard: Mostrar estadísticas generales y vista rápida de pedidos
            updateDashboardStats();
            loadDashboardOrders();
            console.log('📊 Dashboard cargado');
            break;
        case 'orders':
            // Pedidos Activos: Ir directamente a la sección de pedidos activos (estado "Recibido")
            // Esto permite dar seguimiento a todos los pedidos
            showOrderStatusTab('pending');
            renderOrdersTable(); // Renderizar todas las tablas de pedidos por estado
            console.log('📋 Sección de Pedidos Activos cargada');
            break;
        case 'tables':
            // Mesas: Mostrar estado de todas las mesas
            loadTablesStatus();
            console.log('🪑 Estado de Mesas cargado');
            break;
        case 'menu':
            // Menú: Gestión de productos del menú
            loadMenuProducts();
            console.log('📖 Gestión de Menú cargada');
            break;
        case 'inventory':
        ...
        case 'settings':
            ...
            loadSettings();
            console.log('⚙️ Ajustes cargados');
            break;
        case 'qr':
            // Códigos QR: Generación de códigos QR para mesas (sin funcionalidad por ahora)
            loadQRCodes();
            console.log('📱 Códigos QR cargados');
            break;
        case 'notifications':
            // Notificaciones: Ver todas las notificaciones del sistema
            loadNotifications();
...
// Exportar funciones para uso en onclick y eventos
window.navigateToSection = navigateToSection;
window.loadInitialData = loadInitialData;
window.loadActiveOrders = loadActiveOrders;
window.loadTablesStatus = loadTablesStatus;
window.createDefaultTables = createDefaultTables;
...
function initializePanel() {
    ...
    // Forzar carga inicial de pedidos después de un pequeño delay para asegurar que Firebase esté listo
    setTimeout(() => {
        console.log('🔄 Verificando carga de pedidos...');
        if (!ordersListener) {
...
function setupEventListeners() {
    ...
    // Los filtros ahora son pestañas, se manejan con onclick en el HTML
...
async function loadInitialData() {
    try {
        ...
        // Configurar listener de pedidos (esto se conecta en tiempo real)
...
async function loadActiveOrders() {
    // Si ya hay un listener activo, detenerlo primero
    if (ordersListener) {
        ordersListener(); // Detener el listener anterior
        ordersListener = null;
    }
    ...
    try {
        ...
        // Leer TODOS los pedidos (sin filtro para evitar problemas de índice)
        // Filtrar por estado después en el código
        ordersListener = db.collection('orders')
            .onSnapshot((querySnapshot) => {
                console.log('📖 Actualización en tiempo real - Pedidos recibidos:', querySnapshot.size);
                ...
                querySnapshot.forEach(doc => {
                    ...
                    // Solo incluir pedidos activos (excluir cancelled)
                    if (status !== 'cancelled') {
                        activeOrders.push({
                            ID: doc.id,
                            Mesa: data.table || '',
                            Productos: data.products || '',
                            ProductsDetails: data.productsDetails || [], // Detalles completos del pedido
                            Total: data.total || 0,
                            Estado: status,
                            ...
                            rawData: data // Guardar datos completos para el modal
                        });
                    }
                });
                ...
                console.log(`✅ ${activeOrders.length} pedidos activos cargados`);
                ...
                showNotification('Error escuchando pedidos. Verifica la consola.', 'error');
                
                // Intentar cargar pedidos manualmente si el listener falla
                setTimeout(() => {
                    loadActiveOrdersManually();
                }, 3000);
            });
        ...
        await loadActiveOrdersManually();
        return false;
    }
}

// Función de respaldo para cargar pedidos manualmente si el listener falla
async function loadActiveOrdersManually() {
    try {
        console.log('📥 Cargando pedidos manualmente...');
        ...
        console.log(`✅ ${activeOrders.length} pedidos cargados manualmente`);
        ...
        console.error('❌ Error cargando pedidos manualmente:', error);
...
// REGLA: Usar pedidos ya cargados en activeOrders (no hacer otra consulta a Firebase)
async function updateTablesStatusFromOrders() {
    try {
        // Usar pedidos activos que ya están en memoria (no hacer otra consulta)
        const occupiedTables = new Set();
        const tableOrderMap = {};
        
        // Filtrar solo pedidos activos (no entregados)
        activeOrders
            .filter(order => ['pending', 'preparing', 'cooking', 'ready'].includes(order.Estado))
            .forEach(order => {
                const table = order.Mesa || '';
                if (table) {
                    occupiedTables.add(table);
                    tableOrderMap[table] = order.ID;
                }
            });
        ...
        // Solo reflejar el estado actual - NO lógica automática
        tables.forEach(table => {
...
// ============================================
// Renderizar todas las secciones de pedidos por estado
function renderOrdersTable() {
    console.log(`📊 Renderizando pedidos por estado. Total: ${activeOrders.length}`);
    
    // Estados disponibles
    const statuses = ['pending', 'preparing', 'cooking', 'ready', 'delivered'];
    
    // Contar pedidos por estado y renderizar cada sección
    statuses.forEach(status => {
        const ordersByStatus = activeOrders.filter(o => o.Estado === status);
        const tbody = document.getElementById(`ordersBody-${status}`);
        const badge = document.getElementById(`badge-${status}`);
        
        // Actualizar badge con conteo
        if (badge) {
            badge.textContent = ordersByStatus.length;
            badge.style.display = ordersByStatus.length > 0 ? 'inline-block' : 'none';
        }
        
        // Renderizar pedidos de este estado
        if (tbody) {
            if (ordersByStatus.length === 0) {
                    ...
                    <tr>
                        <td colspan="6" class="text-center" style="padding: 40px;">
                            ...
                            <h3 style="color: var(--panel-gray-500); margin-bottom: 8px;">
                                No hay pedidos ${statusLabels[status]}
                            </h3>
                ...
                ordersByStatus.forEach(order => {
                    const statusBadge = getStatusBadge(order.Estado || 'pending');
                    
                    html += `
                        <tr data-order-id="${order.ID}">
                            <td>
                                <div class="font-semibold">${order.ID.substring(0, 12)}...</div>
                                <div class="text-sm text-gray-500">Código: ${order.Código || ''}</div>
                            </td>
                            <td>
                                <div class="flex items-center gap-2">
                                    <span class="text-lg">🪑</span>
                                    <span class="font-semibold">Mesa ${order.Mesa || ''}</span>
                                </div>
                            </td>
                            <td>
                                <div class="text-sm" title="${order.Productos || ''}" style="max-width: 300px;">
                            ...
                            <td>
                                <div class="flex gap-2 flex-wrap">
                                    ...
                                    ` : ''}
                                    ${status === 'delivered' ? `
                                        <span class="status-badge ${statusBadge.class}" style="margin: 0;">
    ...
    console.log(`✅ Pedidos renderizados por estado`);
}

// Mostrar pestaña de estado específico
function showOrderStatusTab(status) {
    // Ocultar todas las secciones
    document.querySelectorAll('.orders-status-section').forEach(section => {
        section.classList.remove('active');
    });
    ...
    // Mostrar sección seleccionada
    const section = document.getElementById(`orders-${status}`);
    if (section) {
...
async function viewOrderDetail(orderId) {
    // Buscar el pedido en activeOrders primero
    let order = activeOrders.find(o => o.ID === orderId);
    
    // Si no está en activeOrders, obtenerlo directamente de Firebase
    if (!order) {
        try {
    ...
    modalBody.innerHTML = `
        <div class="space-y-4">
            ...
            <div>
                <label class="block text-sm font-medium text-gray-500 mb-2">Productos del Pedido</label>
                        ...
                        ? order.ProductsDetails.map(item => {
                            ...
                            return `
                                <div class="flex justify-between items-center py-2 border-b border-gray-200 last:border-0">
                                    <div>
                                        <span class="font-medium">${item.quantity || 1}x ${item.name || ''}</span>
                                        ${item.price ? `<div class="text-xs text-gray-500">$${parseFloat(item.price).toFixed(2)} c/u</div>` : ''}
                                    </div>
                                    <span class="font-semibold text-green-600">$${itemTotal.toFixed(2)}</span>
                                </div>
                            `;
                        }).join('')
                        : (order.Productos || 'Sin productos').split(', ').map(product => `
...
function filterOrders(filter) {
    // Mapear filtros antiguos a nuevos estados
    const statusMap = {
        'all': 'pending',
        'pending': 'pending',
        'preparing': 'preparing',
    ...
    const status = statusMap[filter] || 'pending';
    showOrderStatusTab(status);
}
...
async function loadMenuProducts() {
    ...
    try {
        ...
        if (snapshot.empty) {
            // Si no hay productos, mostrar opción para cargar productos iniciales
            container.innerHTML = `
                <div style="text-align: center; padding: 40px; color: var(--panel-gray-500);">
                    ...
                    <p style="margin-bottom: 20px;">Agrega productos al menú desde el botón "Agregar Producto"</p>
                    <button class="btn-secondary" onclick="initializeProductsFromDefault()" style="margin-top: 10px;">
                        <i class="fas fa-download"></i> Cargar Productos Iniciales
                    </button>
                </div>
            `;
            return;
                ...
                <div class="editable-item">
                    <div class="editable-item-content" style="grid-template-columns: 2fr 1fr 1fr 1fr auto;">
                        <div>
                            ...
                            <textarea value="${data.description || ''}" placeholder="Descripción (ES)" id="product-description-${doc.id}" 
                                   style="width: 100%; padding: 0.5rem; border: 1px solid var(--panel-border); border-radius: var(--panel-radius); margin-top: 0.5rem; font-size: 0.875rem; min-height: 60px; resize: vertical;">${data.description || ''}</textarea>
                            <textarea value="${data.descriptionEn || ''}" placeholder="Descripción (EN)" id="product-descriptionEn-${doc.id}" 
                                   style="width: 100%; padding: 0.5rem; border: 1px solid var(--panel-border); border-radius: var(--panel-radius); margin-top: 0.5rem; font-size: 0.875rem; min-height: 60px; resize: vertical;">${data.descriptionEn || ''}</textarea>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <select id="product-category-${doc.id}" style="width: 100%; padding: 0.5rem; border: 1px solid var(--panel-border); border-radius: var(--panel-radius); margin-top: 0.25rem;">
                                ...
                                <option value="breakfasts" ${data.category === 'breakfasts' ? 'selected' : ''}>Desayunos</option>
                                <option value="mains" ${data.category === 'mains' ? 'selected' : ''}>Platos Principales</option>
...
async function loadSettings() {
    ...
    container.innerHTML = `
        <div style="max-width: 800px; margin: 0 auto;">
            <div class="settings-section" style="background: var(--panel-light); padding: var(--panel-space-6); border-radius: var(--panel-radius-lg); border: 1px solid var(--panel-border); margin-bottom: var(--panel-space-6);">
                <h3 style="margin-bottom: var(--panel-space-4); display: flex; align-items: center; gap: var(--panel-space-3);">
                    <i class="fas fa-info-circle" style="color: var(--panel-info);"></i>
                    Información del Sistema
                </h3>
                <div style="display: grid; gap: var(--panel-space-4);">
                    <div style="display: flex; justify-content: space-between; padding: var(--panel-space-3); background: var(--panel-gray-50); border-radius: var(--panel-radius);">
                        <span style="color: var(--panel-gray-600);">Nombre del Restaurante:</span>
                        <strong>Hunab Ku</strong>
                    </div>
                    <div style="display: flex; justify-content: space-between; padding: var(--panel-space-3); background: var(--panel-gray-50); border-radius: var(--panel-radius);">
                        <span style="color: var(--panel-gray-600);">Total de Mesas:</span>
                        <strong>${PANEL_CONFIG.MAX_TABLES}</strong>
                    </div>
                    <div style="display: flex; justify-content: space-between; padding: var(--panel-space-3); background: var(--panel-gray-50); border-radius: var(--panel-radius);">
                        <span style="color: var(--panel-gray-600);">Estado de Conexión:</span>
                        <strong style="color: var(--panel-success);">
            ...
            <div class="settings-section" style="background: var(--panel-light); padding: var(--panel-space-6); border-radius: var(--panel-radius-lg); border: 1px solid var(--panel-border); margin-bottom: var(--panel-space-6);">
                <h3 style="margin-bottom: var(--panel-space-4); display: flex; align-items: center; gap: var(--panel-space-3);">
                    <i class="fas fa-sync-alt" style="color: var(--panel-primary);"></i>
                    Actualización de Datos
                </h3>
                <div style="display: flex; flex-direction: column; gap: var(--panel-space-3);">
                    <button class="btn btn-primary" onclick="loadInitialData()" style="width: 100%;">
                        <i class="fas fa-sync-alt"></i> Recargar Todos los Datos
                    </button>
                    <button class="btn btn-secondary" onclick="loadActiveOrders()" style="width: 100%;">
                        <i class="fas fa-receipt"></i> Recargar Pedidos
                    </button>
                    <button class="btn btn-secondary" onclick="loadTablesStatus()" style="width: 100%;">
                        <i class="fas fa-chair"></i> Actualizar Estado de Mesas
                    </button>
                </div>
            </div>

            <div class="settings-section" style="background: var(--panel-light); padding: var(--panel-space-6); border-radius: var(--panel-radius-lg); border: 1px solid var(--panel-border);">
                <h3 style="margin-bottom: var(--panel-space-4); display: flex; align-items: center; gap: var(--panel-space-3);">
                    <i class="fas fa-database" style="color: var(--panel-warning);"></i>
                    Gestión de Datos
                </h3>
                <div style="display: flex; flex-direction: column; gap: var(--panel-space-3);">
                    <p style="color: var(--panel-gray-600); font-size: 0.875rem; margin-bottom: var(--panel-space-4);">
                        Aquí puedes gestionar los datos del sistema. Todas las acciones son inmediatas y se reflejan en tiempo real.
                    </p>
                    <button class="btn btn-secondary" onclick="createDefaultTables()" style="width: 100%;">
                        <i class="fas fa-chair"></i> Recrear Mesas por Defecto
                    </button>
                    <p style="color: var(--panel-gray-500); font-size: 0.75rem; margin-top: var(--panel-space-2);">
                        Esto solo creará las mesas si no existen en Firebase.
                    </p>
                </div>
            </div>
        </div>
...
// Inicializar productos desde array por defecto (productos del restaurante)
async function initializeProductsFromDefault() {
    // Productos por defecto del restaurante Hunab Ku
    ...
    try {
        showNotification('Cargando productos iniciales...', 'info');
        
        // Verificar si ya existen productos
        const existingSnapshot = await db.collection('products').get();
        if (!existingSnapshot.empty) {
            if (!confirm('Ya existen productos en Firebase. ¿Deseas agregar estos productos además de los existentes?')) {
                return;
            }
        }
        ...
        await batch.commit();
        showNotification(`✅ ${count} productos cargados exitosamente`, 'success');
        loadMenuProducts();
...
async function loadReports(period = 'day') {
    ...
    container.innerHTML = '<div style="text-align: center; padding: 40px;"><i class="fas fa-chart-line fa-spin" style="font-size: 2rem; color: var(--panel-primary);"></i><p>Generando reportes...</p></div>';
    ...
    try {
        ...
        // Procesar datos
        const reportData = await processReportData(snapshot, period, startDate, endDate);
        
        // Mostrar reportes
        container.innerHTML = generateReportsHTML(reportData, periodLabel);
        
        // Configurar eventos de los botones
        setupReportFilters(period);
        
        // Generar gráficos
        generateCharts(reportData, period);
        
    } catch (error) {
        console.error('Error cargando reportes:', error);
        container.innerHTML = '<p style="color: var(--panel-danger); text-align: center; padding: 40px;">Error cargando reportes. Intenta nuevamente.</p>';
    }
}

async function processReportData(snapshot, period, startDate, endDate) {
    const data = {
        totalSales: 0,
    ...
    snapshot.forEach(doc => {
        ...
        // Total de ventas
        data.totalSales += orderTotal;
        
        // Por hora del día
        const hour = orderDate.getHours();
        data.ordersByHour[hour] = (data.ordersByHour[hour] || 0) + 1;
        
        // Por día (para gráficos)
        const dayKey = orderDate.toISOString().split('T')[0];
        if (!data.salesByDay[dayKey]) {
            data.salesByDay[dayKey] = {
                date: dayKey,
                sales: 0,
                orders: 0
            };
        }
        data.salesByDay[dayKey].sales += orderTotal;
        data.salesByDay[dayKey].orders += 1;
        ...
        if (order.productsDetails && Array.isArray(order.productsDetails)) {
            order.productsDetails.forEach(item => {
                ...
                allProducts[productName].quantity += quantity;
                allProducts[productName].revenue += (parseFloat(item.price || 0) * quantity);
            });
        }
    });
    
    // Convertir productos a array y ordenar
    data.topProducts = Object.values(allProducts)
        .sort((a, b) => b.quantity - a.quantity)
        .slice(0, 10); // Top 10 productos
    
    // Calcular ticket promedio
    data.averageTicket = data.totalOrders > 0 ? data.totalSales / data.totalOrders : 0;
    
    // Ordenar timeline por fecha (más reciente primero)
    data.timeline.sort((a, b) => b.date - a.date);
    
    return data;
}

function generateReportsHTML(data, periodLabel) {
    return `
        <div class="reports-container">
            ...
            <div class="stats-grid" style="margin-bottom: 30px;">
                ...
                <div class="stat-card">
                    <div class="stat-header">
                        ...
                        <span class="stat-title">Pedidos ${periodLabel}</span>
                    </div>
                ...
                <div class="stat-card">
                    ...
                    <div class="stat-value">$${data.averageTicket.toFixed(2)}</div>
                ...
                <div class="stat-card">
                    <div class="stat-header">
                        ...
                        <span class="stat-title">Productos Vendidos</span>
                    </div>
                    <div class="stat-value">${data.topProducts.reduce((sum, p) => sum + p.quantity, 0)}</div>
                    <div class="stat-change positive">
                        <i class="fas fa-fire"></i>
                        ${data.topProducts.length} diferentes
                    </div>
                </div>
            </div>
            ...
            <div class="charts-grid" style="margin-bottom: 30px;">
                ...
                <div class="chart-card">
                    <div class="chart-header">
                        <h3><i class="fas fa-chart-bar"></i> Productos Más Vendidos</h3>
                        <div class="chart-actions">
                            <button class="chart-btn active" onclick="switchProductChart('quantity')">
                                <i class="fas fa-sort-amount-down"></i> Cantidad
                            </button>
                            <button class="chart-btn" onclick="switchProductChart('revenue')">
                                <i class="fas fa-money-bill"></i> Ingresos
                            </button>
                        </div>
                    </div>
                    <div class="chart-container">
            ...
            <div class="section-card" style="margin-bottom: 30px;">
                <div class="section-header">
                    <h3><i class="fas fa-crown"></i> Top 10 Productos</h3>
                        ...
                        <tbody id="topProductsTable">
                            ${data.topProducts.map((product, index) => `
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <span class="quantity-badge">${product.quantity} unidades</span>
                                    </td>
                                    <td class="text-success font-bold">
                                        $${product.revenue.toFixed(2)}
                                    </td>
                                    <td class="text-muted">
                                        $${(product.revenue / product.quantity).toFixed(2)} c/u
                                    </td>
                                </tr>
                            `).join('')}
                        </tbody>
                ...
                <div class="table-responsive">
                    <table class="reports-table">
                        ...
                        <tbody id="salesHistoryTable">
                            ${data.timeline.slice(0, 20).map(order => `
                                <tr>
                                    <td>
                                        <div class="text-sm">${order.date.toLocaleDateString()}</div>
                                        <div class="text-xs text-muted">${order.date.toLocaleTimeString()}</div>
                                    </td>
                                    <td>
                                        <code class="text-xs">${order.id.substring(0, 8)}...</code>
                                    </td>
                                    <td>
                                        <span class="badge badge-secondary">Mesa ${order.table}</span>
                                    </td>
                                    <td class="text-sm" style="max-width: 200px;">
                                        ${order.products.substring(0, 50)}${order.products.length > 50 ? '...' : ''}
                                    </td>
                                    <td>
                ...
                ${data.timeline.length > 20 ? `
                    <div class="section-footer">
                        <p class="text-center text-muted">
                            Mostrando 20 de ${data.timeline.length} pedidos
                        </p>
                    </div>
                ` : ''}
            </div>
            ...
            <div class="section-card" style="margin-top: 30px;">
                <div class="section-header">
                    <h3><i class="fas fa-chart-pie"></i> Resumen Estadístico</h3>
                </div>
                <div class="stats-summary">
                    <div class="stat-item">
                        <div class="stat-label">Venta Más Alta</div>
                        <div class="stat-value text-success">
                            $${data.timeline.length > 0 ? Math.max(...data.timeline.map(o => o.total)).toFixed(2) : '0.00'}
                        </div>
                    </div>
                    <div class="stat-item">
                        <div class="stat-label">Venta Más Baja</div>
                        <div class="stat-value text-warning">
                            $${data.timeline.length > 0 ? Math.min(...data.timeline.map(o => o.total)).toFixed(2) : '0.00'}
                        </div>
                    </div>
                    <div class="stat-item">
                        <div class="stat-label">Pedidos por Hora (Prom.)</div>
                        <div class="stat-value">
                            ${Object.keys(data.ordersByHour).length > 0 ? 
                              (data.totalOrders / Object.keys(data.ordersByHour).length).toFixed(1) : '0'}
                        </div>
                    </div>
                    <div class="stat-item">
                        ...
                        <div class="stat-value">
                            ${Object.keys(data.ordersByHour).length > 0 ? 
                              Object.entries(data.ordersByHour).sort((a, b) => b[1] - a[1])[0][0] + ':00 hrs' : 'N/A'}
                        </div>
                    </div>
                </div>
            </div>
...
function setupReportFilters(activePeriod) {
    ...
    periods.forEach(period => {
        ...
        if (btn) {
            ...
            btn.onclick = () => loadReports(period);
        }
    });
}

function generateCharts(data, period) {
    // Gráfico de ventas por día
    const salesCtx = document.getElementById('salesChart');
    if (salesCtx) {
        const salesByDay = Object.values(data.salesByDay)
        ...
        new Chart(salesCtx.getContext('2d'), {
            ...
            options: {
                ...
                plugins: {
                    ...
                    title: {
                        display: true,
                        text: `Ventas por Día - ${period === 'day' ? 'Hoy' : period === 'week' ? 'Esta Semana' : 'Este Mes'}`
                    }
                },
                scales: {
                    y: {
    ...
    if (productsCtx) {
        generateProductsChart(data.topProducts, 'quantity');
    }
}

function generateProductsChart(products, type = 'quantity') {
    const ctx = document.getElementById('productsChart');
    if (!ctx) return;
    
    const sortedProducts = [...products].sort((a, b) => {
        return type === 'quantity' ? b.quantity - a.quantity : b.revenue - a.revenue;
    ...
    new Chart(ctx.getContext('2d'), {
        ...
        data: {
            ...
            datasets: [{
                label: type === 'quantity' ? 'Cantidad Vendida' : 'Ingresos ($)',
                data: sortedProducts.map(p => type === 'quantity' ? p.quantity : p.revenue),
                backgroundColor: sortedProducts.map((_, i) => 
                    `hsl(${i * 40}, 70%, 60%)`
                ),
                borderColor: sortedProducts.map((_, i) => 
                    `hsl(${i * 40}, 70%, 40%)`
                ),
                borderWidth: 1
            }]
        ...
        options: {
            ...
            plugins: {
                ...
                title: {
                    display: true,
                    text: type === 'quantity' ? 'Productos Más Vendidos (Cantidad)' : 'Productos Más Rentables (Ingresos)'
                },
                tooltip: {
                    callbacks: {
                        label: function(context) {
                            ...
                            if (type === 'quantity') {
                                return `${product.quantity} unidades - $${product.revenue.toFixed(2)}`;
                            }
                            return `$${product.revenue.toFixed(2)} - ${product.quantity} unidades`;
                        }
                    }
                }
            },
...
function switchProductChart(type) {
    const buttons = document.querySelectorAll('#section-reports .chart-btn');
    buttons.forEach(btn => btn.classList.remove('active'));
    
    const activeBtn = document.querySelector(`#section-reports .chart-btn[onclick*="${type}"]`);
    if (activeBtn) activeBtn.classList.add('active');
    
    // Obtener datos actuales de la tabla
    const products = [];
    document.querySelectorAll('#topProductsTable tr').forEach(row => {
        const name = row.querySelector('.product-name')?.textContent;
        const quantity = parseInt(row.querySelector('.quantity-badge')?.textContent || '0');
        const revenue = parseFloat(row.querySelector('.text-success')?.textContent?.replace('$', '') || '0');
        
        if (name && quantity) {
            products.push({ name, quantity, revenue });
        }
    });
    
    generateProductsChart(products, type);
}

function exportToExcel() {
    const table = document.querySelector('#salesHistoryTable');
    if (!table) {
        showNotification('No hay datos para exportar', 'warning');
        return;
    }
    
    let csv = 'Fecha,Hora,ID Pedido,Mesa,Productos,Código,Total\n';
    ...
    table.querySelectorAll('tr').forEach(row => {
        ...
        if (cells.length >= 6) {
            ...
            const tableNum = cells[2].textContent.replace('Mesa ', '').trim();
            const products = cells[3].textContent.trim().replace(/,/g, ';');
            const code = cells[4].textContent.trim();
            const total = cells[5].textContent.replace('$', '').trim();
            
            csv += `"${date}","${time}","${id}","${tableNum}","${products}","${code}","${total}"\n`;
        }
    });
    ...
    link.setAttribute('download', `ventas_hunabku_${new Date().toISOString().split('T')[0]}.csv`);
...
// Modificar la función loadSectionData para incluir reportes
// (Agrega esto a la función loadSectionData existente en panel.js)
// case 'reports':
//     loadReports('day');
//     console.log('📊 Reportes cargados');
//     break;
...
window.initializeProductsFromDefault = initializeProductsFromDefault;
...
window.showOrderStatusTab = showOrderStatusTab;
