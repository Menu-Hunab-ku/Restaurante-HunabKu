...
async function initialize() {
    ...
    try {
        ...
        showNotification('Error cargando menú. Usando datos locales.', 'warning');
        // Usar datos locales como respaldo
        if (PRODUCTS.length === 0) {
            PRODUCTS = [
                { id: 1, name: "Filete Mignon", description: "Corte premium 250g con salsa de vino tinto y vegetales asados", price: 34.99, category: "mains", image: "https://images.unsplash.com/photo-1546833999-b9f581a1996d?w=400&h=300&fit=crop", featured: true }
            ];
        }
...
async function loadCategoriesFromFirebase() {
    try {
        ...
        CATEGORIES = [
            ...
            { id: 'appetizers', name: currentLanguage === 'es' ? 'Entradas' : 'Appetizers', icon: 'fas fa-seedling', color: '#27AE60' },
            { id: 'mains', name: currentLanguage === 'es' ? 'Platos Principales' : 'Main Dishes', icon: 'fas fa-drumstick-bite', color: '#E74C3C' },
            { id: 'desserts', name: currentLanguage === 'es' ? 'Postres' : 'Desserts', icon: 'fas fa-ice-cream', color: '#9B59B6' },
            { id: 'drinks', name: currentLanguage === 'es' ? 'Bebidas' : 'Drinks', icon: 'fas fa-glass-cheers', color: '#3498DB' },
            { id: 'specials', name: currentLanguage === 'es' ? 'Especiales' : 'Specials', icon: 'fas fa-crown', color: '#D4AF37' }
        ];
        return false;
    }
}
    ...
    if (PRODUCTS.length === 0) {
        container.innerHTML = `
            <div style="grid-column: 1 / -1; text-align: center; padding: 60px 20px; color: var(--gray-500);">
                ...
                <h3>${currentLanguage === 'es' ? 'Cargando productos...' : 'Loading products...'}</h3>
                <p>${currentLanguage === 'es' ? 'Por favor espera mientras cargamos el menú' : 'Please wait while we load the menu'}</p>
            </div>
        `;
        return;
    }
