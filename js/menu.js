...
function setupOrderStatusListener(orderId) {
    ...
    const unsubscribe = db.collection('orders').doc(orderId).onSnapshot((doc) => {
        if (doc.exists()) {
            ...
            // Si los pasos no existen, renderizarlos primero
            const trackingSteps = document.getElementById('trackingSteps');
            if (!trackingSteps || trackingSteps.children.length === 0) {
                renderOrderTrackingSteps();
            }
            
            // Actualizar pasos según el estado (desactivar todos)
            ...
            // Actualizar el estado en currentOrder
            if (currentOrder) {
                currentOrder.status = status;
            }
            
            // Si el pedido fue entregado, mostrar factura inmediatamente
            if (status === 'delivered') {
                console.log('✅ Pedido entregado, mostrando factura...');
                ...
                // Mostrar factura inmediatamente
                if (currentOrder) {
                    // Usar currentOrder existente
                    showInvoice();
                } else {
                    // Si no hay currentOrder, reconstruirlo desde Firebase
                    const orderData = doc.data();
                    if (orderData && orderData.productsDetails) {
                        console.log('📦 Reconstruyendo pedido desde Firebase para factura...');
                        // Reconstruir currentOrder desde Firebase
                        currentOrder = {
                            id: doc.id,
                            table: orderData.table || '',
                            code: orderData.code || '',
                            timestamp: orderData.createdAt ? orderData.createdAt.toDate() : new Date(),
                            cart: orderData.productsDetails.map(item => ({
                                id: item.id || '',
                                name: item.name || '',
                                price: item.price || 0,
                        ...
                        console.warn('⚠️ No se pudo reconstruir el pedido para la factura');
                    }
                }
                
                // Dejar de escuchar después de mostrar factura
                setTimeout(() => {
                    if (unsubscribe) unsubscribe();
                    console.log('👋 Listener de estado del pedido detenido');
                }, 2000);
            }
        } else {
            console.warn('⚠️ Pedido no encontrado en Firebase');
        }
    }, (error) => {
    ...
    return unsubscribe; // Devolver función para poder cancelar si es necesario
}
