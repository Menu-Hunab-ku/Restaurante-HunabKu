...
async function initialize() {
    try {
        ...
        // SIEMPRE mostrar productos locales INMEDIATAMENTE (sin esperar nada)
        console.log(`📦 Productos locales disponibles: ${PRODUCTS.length}`);
        if (PRODUCTS.length > 0) {
            ...
            console.log(`✅ ${PRODUCTS_FROM_FIREBASE.length} productos locales cargados`);
            renderProducts(); // Renderizar inmediatamente con productos locales
        } else {
            console.warn('⚠️ No hay productos locales definidos');
        ...
        // NO bloquear la UI inicial
        setTimeout(() => {
            loadProductsFromFirebase().then(() => {
                // Solo actualizar si Firebase tiene productos
                if (PRODUCTS_FROM_FIREBASE.length > 0) {
                    renderProducts();
                    console.log('✅ Productos actualizados desde Firebase');
                }
            }).catch(error => {
                console.warn('No se pudieron cargar productos desde Firebase, continuando con locales:', error);
            });
        }, 500); // Delay pequeño para no interferir con la carga inicial
        
        // Verificar conexión Firebase (solo para logs) - en segundo plano
        setTimeout(() => {
        ...
        // Aún así, intentar mostrar productos locales
        if (PRODUCTS.length > 0) {
            PRODUCTS_FROM_FIREBASE = PRODUCTS.map((p, index) => ({
                id: p.id || `local-${index}`,
                ...p,
...
async function loadProductsFromFirebase() {
    try {
        ...
        if (typeof db === 'undefined' || !db) {
            console.error('❌ Firebase no está disponible - Verifica la configuración');
        ...
        const timeoutPromise = new Promise((_, reject) => 
            setTimeout(() => reject(new Error('Timeout: Firebase tardó demasiado')), 5000)
        );
        
        const snapshotPromise = db.collection('products').get();
        ...
        // PERO NO bloquear aquí - hacerlo en segundo plano para no detener la carga
        if (PRODUCTS_FROM_FIREBASE.length === 0 && PRODUCTS.length > 0) {
            ...
            // NO usar await aquí - hacerlo en segundo plano
            initializeProductsInFirestore().then(() => {
                // Recargar después de inicializar (en segundo plano)
                db.collection('products').get().then(reloadSnapshot => {
                    const newProducts = [];
                    ...
                    // Actualizar solo si hay productos nuevos
                    if (newProducts.length > 0) {
                        PRODUCTS_FROM_FIREBASE = newProducts;
                        renderProducts(); // Actualizar UI
                        console.log(`✅ ${newProducts.length} productos inicializados desde Firestore`);
                    }
                }).catch(err => {
                    console.warn('Error recargando productos después de inicializar:', err);
                });
            }).catch(err => {
                console.warn('Error inicializando productos en Firestore (continuando con locales):', err);
            });
        }
...
async function testFirebaseConnection() {
    try {
        ...
        // Intentar leer la colección de productos (que debe existir)
        const testDoc = await db.collection('products').limit(1).get();
        console.log('✅ Conexión a Firebase exitosa');
        return true;
    } catch (error) {
        console.error('❌ Error de conexión con Firebase:', error);
        // NO mostrar notificación de modo local - solo log en consola
        return false;
    }
}
